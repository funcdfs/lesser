...
type mockBanRepository struct {
	bans map[string]*repository.Ban // userID -> ban
}

func newMockBanRepository() *mockBanRepository {
	return &mockBanRepository{
		bans: make(map[string]*repository.Ban),
	}
}

func (m *mockBanRepository) Create(ctx context.Context, ban *repository.Ban) error {
	m.bans[ban.UserID] = ban
	return nil
}

func (m *mockBanRepository) GetByUserID(ctx context.Context, userID string) (*repository.Ban, error) {
...
func (m *mockBanRepository) IsUserBanned(ctx context.Context, userID string) (bool, *repository.Ban, error) {
	ban, ok := m.bans[userID]
	if !ok {
...
func TestLogin_BannedUser(t *testing.T) {
	...
	// 封禁用户
	err = banRepo.Create(ctx, &repository.Ban{
		ID:        "ban-1",
