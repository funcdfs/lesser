...
func (s *AuthServiceImpl) Register(ctx context.Context, username, email, password, displayName string) (*AuthResult, error) {
	...
	if exists {
		return nil, data_access.ErrUserExists
	}
...
func (s *AuthServiceImpl) CheckBanned(ctx context.Context, userID string) (*BanInfo, error) {
	...
	// 写入缓存
	if s.banCache != nil {
		_ = s.banCache.Set(ctx, userID, &data_access.BanCacheEntry{
			Banned:    info.Banned,
			Reason:    info.Reason,
			ExpiresAt: info.ExpiresAt,
		})
...
func (s *AuthServiceImpl) generateTokens(user *data_access.User) (*AuthResult, error) {
