...
// BanRepositoryImpl PostgreSQL 封禁仓库实现
type BanRepositoryImpl struct {
	db *sql.DB
}

// NewBanRepository 创建封禁仓库
func NewBanRepository(db *sql.DB) *BanRepositoryImpl {
	return &BanRepositoryImpl{db: db}
}

// Create 创建封禁记录
func (r *BanRepositoryImpl) Create(ctx context.Context, ban *Ban) error {
	// 先将该用户的旧封禁记录设为非活跃
...
func (r *BanRepositoryImpl) GetByUserID(ctx context.Context, userID string) (*Ban, error) {
	query := `
...
func (r *BanRepositoryImpl) Delete(ctx context.Context, userID string) error {
...
func (r *BanRepositoryImpl) IsUserBanned(ctx context.Context, userID string) (bool, *Ban, error) {
...
// 确保实现接口
var _ BanRepository = (*BanRepositoryImpl)(nil)
