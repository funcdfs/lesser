...
// RepostDataAccess 转发数据访问
type RepostDataAccess struct {
	db *sql.DB
}

// NewRepostDataAccess 创建转发数据访问实例
func NewRepostDataAccess(db *sql.DB) *RepostDataAccess {
	return &RepostDataAccess{db: db}
}

// Create 创建转发记录
// 返回值：创建的转发记录、是否实际插入了新记录、错误
func (r *RepostDataAccess) Create(userID, contentID, quote string) (*Repost, bool, error) {
	repost := &Repost{
		ID:        uuid.New().String(),
		UserID:    userID,
		ContentID: contentID,
...
// Delete 删除转发记录
// 返回值：是否实际删除了记录、错误
func (r *RepostDataAccess) Delete(userID, contentID string) (bool, error) {
...
// Exists 检查是否已转发
func (r *RepostDataAccess) Exists(userID, contentID string) (bool, error) {
	var exists bool
...
func (r *RepostDataAccess) BatchExists(userID string, contentIDs []string) (map[string]bool, error) {
...
func (r *RepostDataAccess) GetByUserAndContent(userID, contentID string) (*Repost, error) {
	repost := &Repost{}
