...
// BookmarkDataAccess 收藏数据访问
type BookmarkDataAccess struct {
	db *sql.DB
}

// NewBookmarkDataAccess 创建收藏数据访问实例
func NewBookmarkDataAccess(db *sql.DB) *BookmarkDataAccess {
	return &BookmarkDataAccess{db: db}
}

// Create 创建收藏记录
// 返回值：是否实际插入了新记录、错误
func (r *BookmarkDataAccess) Create(userID, contentID string) (bool, error) {
...
// Delete 删除收藏记录
// 返回值：是否实际删除了记录、错误
func (r *BookmarkDataAccess) Delete(userID, contentID string) (bool, error) {
...
// Exists 检查是否已收藏
func (r *BookmarkDataAccess) Exists(userID, contentID string) (bool, error) {
	var exists bool
...
func (r *BookmarkDataAccess) BatchExists(userID string, contentIDs []string) (map[string]bool, error) {
...
// List 获取收藏列表
func (r *BookmarkDataAccess) List(userID string, limit, offset int) ([]*Bookmark, int, error) {
	var total int
