...
func TestGRPCClientPool(t *testing.T) {
	...
	// 注册测试服务配置
	pool.Register("gateway", client.Config{
		Target:       gatewayAddr,
		Insecure:     true,
...
func TestTraceIDPropagation(t *testing.T) {
	...
	ctx := log.ContextWithTraceID(context.Background(), traceID)

	// 验证 trace_id 可以从 context 中提取
	extractedTraceID := log.TraceIDFromContext(ctx)
