...
import (
	...
	"github.com/funcdfs/lesser/pkg/middleware"
...
func (s *Server) Build(unaryInterceptors []grpc.UnaryServerInterceptor, streamInterceptors []grpc.StreamServerInterceptor) *grpc.Server {
	...
	defaultUnary := []grpc.UnaryServerInterceptor{
		middleware.RecoveryInterceptor(s.log),
		middleware.TraceInterceptor(),
		middleware.LoggingInterceptor(s.log),
	}

	defaultStream := []grpc.StreamServerInterceptor{
		middleware.StreamRecoveryInterceptor(s.log),
		middleware.StreamTraceInterceptor(),
		middleware.StreamLoggingInterceptor(s.log),
	}
