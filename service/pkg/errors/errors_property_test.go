	...
	properties.Property("Handler 层应使用 logic.ToGRPCError 或 h.handleError 进行错误转换", prop.ForAll(
		func(serviceName string) bool {
			...
			for _, entry := range entries {
				...
				// 检查是否有内联的 status.Error 调用（不在参数验证中）
				// 允许的模式：
				// 1. logic.ToGRPCError(err)
				// 2. h.handleError(err) - 如果 handleError 内部调用 logic.ToGRPCError
				// 3. mapError(err) - 如果 mapError 内部调用 logic.ToGRPCError
				// 4. status.Error 用于参数验证（如 "不能为空"）
				...
				if hasErrorHandling && !usesToGRPCError && !hasLogicImport {
					// 检查是否只是参数验证错误
					if !isOnlyParameterValidation(contentStr) {
...
// isOnlyParameterValidation 检查文件是否只包含参数验证错误
func isOnlyParameterValidation(content string) bool {
...
func TestProperty12_ErrorMessagesInChinese(t *testing.T) {
	...
	properties.Property("logic/errors.go 中的 ToGRPCError 应返回中文错误消息", prop.ForAll(
		func(serviceName string) bool {
			...
			// 简单检查：文件中应该包含中文字符
			hasChinese := containsChinese(contentStr)
