...
func (m *mockCommentRepository) Create(ctx context.Context, comment *repository.Comment) error {
	if m.createErr != nil {
		return m.createErr
	}
	// 验证父评论
	if comment.ParentID != "" {
		...
		if !exists || parent.IsDeleted {
			return repository.ErrInvalidParent
		}
		parent.ReplyCount++
	}
...
func (m *mockCommentRepository) GetByID(ctx context.Context, id string) (*repository.Comment, error) {
	if m.getErr != nil {
		return nil, m.getErr
	}
...
func (m *mockCommentRepository) Delete(ctx context.Context, id string) (*repository.Comment, error) {
	if m.deleteErr != nil {
...
func (m *mockCommentRepository) List(ctx context.Context, contentID, parentID string, sortBy repository.SortBy, limit, offset int) ([]*repository.Comment, int, error) {
	if m.listErr != nil {
