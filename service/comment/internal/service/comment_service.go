// Package service 提供 Comment 服务的业务逻辑层
package service
...
// CreateComment 创建评论
func (s *CommentService) CreateComment(ctx context.Context, authorID, contentID, parentID, text string) (*repository.Comment, int32, error) {
	if text == "" {
	...
	if err := s.commentRepo.Create(ctx, comment); err != nil {
		if err == repository.ErrInvalidParent {
			return nil, 0, ErrInvalidParent
		}
		return nil, 0, err
	}
	...
	if s.publisher != nil {
		...
		s.publisher.PublishAsync(ctx, broker.EventCommentCreated, event)
	}
...
// GetComment 获取单条评论
func (s *CommentService) GetComment(ctx context.Context, commentID string) (*repository.Comment, error) {
...
func (s *CommentService) ListComments(ctx context.Context, contentID, parentID string, sortBy SortBy, limit, offset int) ([]*repository.Comment, int, error) {
	if limit <= 0 {
		limit = 20
	}
	if limit > 100 {
	...
	return s.commentRepo.List(ctx, contentID, parentID, repository.SortBy(sortBy), limit, offset)
}
...
func (s *CommentService) LikeComment(ctx context.Context, userID, commentID string) (int32, error) {
	...
	if s.publisher != nil && comment.AuthorID != userID {
		...
		s.publisher.PublishAsync(ctx, broker.EventCommentLiked, event)
	}
