...
type propertyTestCommentRepo struct {
	comments map[string]*repository.Comment
}

func newPropertyTestCommentRepo() *propertyTestCommentRepo {
	return &propertyTestCommentRepo{
		comments: make(map[string]*repository.Comment),
	}
}

func (r *propertyTestCommentRepo) Create(ctx context.Context, comment *repository.Comment) error {
...
func (r *propertyTestCommentRepo) GetByID(ctx context.Context, id string) (*repository.Comment, error) {
...
func (r *propertyTestCommentRepo) Delete(ctx context.Context, id string) (*repository.Comment, error) {
	comment, exists := r.comments[id]
	if !exists {
		return nil, repository.ErrCommentNotFound
	}
...
func (r *propertyTestCommentRepo) List(ctx context.Context, contentID, parentID string, sortBy repository.SortBy, limit, offset int) ([]*repository.Comment, int, error) {
	...
	case repository.SortByNewest, repository.SortByUnspecified:
		fallthrough
	default:
		sort.Slice(result, func(i, j int) bool {
			return result[i].CreatedAt.After(result[j].CreatedAt)
	...
	total := len(result)
	if offset >= total {
		return []*repository.Comment{}, total, nil
	}
	end := offset + limit
	if end > total {
		end = total
...
func (r *propertyTestCommentRepo) LikeComment(ctx context.Context, userID, commentID string) (int32, error) {
	...
	if !exists {
		return 0, repository.ErrCommentNotFound
	}
	comment.LikeCount++
...
func (r *propertyTestCommentRepoWithLikes) Create(ctx context.Context, comment *repository.Comment) error {
	comment.ID = "comment-" + time.Now().Format("20060102150405.000000000")
	comment.CreatedAt = time.Now()
	comment.UpdatedAt = time.Now()
	r.comments[comment.ID] = comment
...
func (r *propertyTestCommentRepoWithLikes) GetByID(ctx context.Context, id string) (*repository.Comment, error) {
	comment, exists := r.comments[id]
	if !exists {
		return nil, repository.ErrCommentNotFound
	}
...
func (r *propertyTestCommentRepoWithLikes) Delete(ctx context.Context, id string) (*repository.Comment, error) {
	comment, exists := r.comments[id]
	if !exists {
		return nil, repository.ErrCommentNotFound
	}
...
func (r *propertyTestCommentRepoWithLikes) List(ctx context.Context, contentID, parentID string, sortBy repository.SortBy, limit, offset int) ([]*repository.Comment, int, error) {
	var result []*repository.Comment
	for _, c := range r.comments {
...
func (r *propertyTestCommentRepoWithLikes) LikeComment(ctx context.Context, userID, commentID string) (int32, error) {
	...
	if r.commentLikes[commentID][userID] {
		return 0, repository.ErrAlreadyLiked
	}
...
func (r *propertyTestCommentRepoWithLikes) UnlikeComment(ctx context.Context, userID, commentID string) (int32, error) {
	...
	if r.commentLikes[commentID] == nil || !r.commentLikes[commentID][userID] {
		return 0, repository.ErrNotLiked
	}
