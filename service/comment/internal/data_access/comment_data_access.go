...
// CommentDataAccess 评论数据访问
type CommentDataAccess struct {
	db  *sql.DB
	log *log.Logger
}

// NewCommentDataAccess 创建评论数据访问实例
func NewCommentDataAccess(db *sql.DB) *CommentDataAccess {
	return &CommentDataAccess{
		db:  db,
		log: log.Global().With(log.String("component", "comment_data_access")),
	}
}

// NewCommentDataAccessWithLogger 创建带日志的评论数据访问实例
func NewCommentDataAccessWithLogger(db *sql.DB, logger *log.Logger) *CommentDataAccess {
	if logger == nil {
		logger = log.Global()
	}
	return &CommentDataAccess{
		db:  db,
		log: logger.With(log.String("component", "comment_data_access")),
	}
}

// Create 创建评论记录
func (r *CommentDataAccess) Create(ctx context.Context, comment *Comment) error {
...
// GetByID 根据 ID 获取评论
func (r *CommentDataAccess) GetByID(ctx context.Context, id string) (*Comment, error) {
	comment := &Comment{}
...
// List 获取评论列表（支持多种排序）
func (r *CommentDataAccess) List(ctx context.Context, contentID, parentID string, sortBy SortBy, limit, offset int) ([]*Comment, int, error) {
	var total int
	...
	if err := r.db.QueryRowContext(ctx, countQuery, countArgs...).Scan(&total); err != nil {
		r.log.Error("统计评论总数失败",
			log.String("content_id", contentID),
			log.Any("error", err))
...
// scanComments 扫描多行评论记录
func (r *CommentDataAccess) scanComments(rows *sql.Rows) ([]*Comment, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ID, &c.AuthorID, &c.ContentID, &pID, &c.Text, &c.IsDeleted, &c.CreatedAt, &c.UpdatedAt, &c.ReplyCount, &c.LikeCount); err != nil {
			r.log.Error("扫描评论行失败", log.Any("error", err))
...
func (r *CommentDataAccess) buildOrderClause(sortBy SortBy, isReply bool) string {
	switch sortBy {
	case SortByOldest:
		// 最早优先
...
// Delete 软删除评论
func (r *CommentDataAccess) Delete(ctx context.Context, id string) (*Comment, error) {
...
// GetCount 获取内容的评论数量
func (r *CommentDataAccess) GetCount(ctx context.Context, contentID string) (int32, error) {
	var count int32
	err := r.db.QueryRowContext(ctx, `
...
func (r *CommentDataAccess) BatchGetCount(ctx context.Context, contentIDs []string) (map[string]int32, error) {
...
// LikeComment 点赞评论
func (r *CommentDataAccess) LikeComment(ctx context.Context, userID, commentID string) (int32, error) {
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return 0, err
	}
...
func (r *CommentDataAccess) UnlikeComment(ctx context.Context, userID, commentID string) (int32, error) {
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return 0, err
	}
...
func (r *CommentDataAccess) CheckLiked(ctx context.Context, userID, commentID string) (bool, error) {
	var exists bool
	err := r.db.QueryRowContext(ctx, `
		SELECT EXISTS(SELECT 1 FROM comment_likes WHERE user_id = $1 AND comment_id = $2)
...
func (r *CommentDataAccess) BatchCheckLiked(ctx context.Context, userID string, commentIDs []string) (map[string]bool, error) {
