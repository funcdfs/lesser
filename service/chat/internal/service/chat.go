// Package service 提供 Chat 服务的业务逻辑层
package service
...
type CreateConversationRequest struct {
	Type      repository.ConversationType
	Name      string
	MemberIDs []uuid.UUID
	CreatorID uuid.UUID
}
...
func (r *CreateConversationRequest) Validate() error {
	...
	if r.Type == repository.ConversationTypePrivate && len(r.MemberIDs) != 2 {
		return ErrPrivateMemberCount
	}
	if r.Type == repository.ConversationTypeGroup && r.Name == "" {
...
func (s *ChatService) CreateConversation(ctx context.Context, req CreateConversationRequest) (*repository.Conversation, error) {
	if err := req.Validate(); err != nil {
		return nil, err
	}

	// 私聊会话检查是否已存在
	if req.Type == repository.ConversationTypePrivate && len(req.MemberIDs) == 2 {
...
func (s *ChatService) GetConversation(ctx context.Context, id uuid.UUID) (*repository.Conversation, error) {
...
// SendMessage 发送消息
func (s *ChatService) SendMessage(ctx context.Context, req SendMessageRequest) (*repository.Message, error) {
	if err := req.Validate(); err != nil {
		return nil, err
	}
...
func (s *ChatService) AddMember(ctx context.Context, conversationID, userID, addedBy uuid.UUID) error {
	...
	if conv.Type == repository.ConversationTypePrivate {
		return ErrCannotAddToPrivate
	}

	if err := s.conversationRepo.AddMember(ctx, conversationID, userID, repository.MemberRoleMember); err != nil {
...
func (s *ChatService) MarkConversationAsRead(ctx context.Context, conversationID, userID uuid.UUID) (*repository.BatchReadReceipt, error) {
	...
	return &repository.BatchReadReceipt{
...
func (s *ChatService) populateMemberUserInfo(ctx context.Context, conv *repository.Conversation) {
