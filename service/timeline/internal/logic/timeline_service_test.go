...
func createTestFeedItem(id, authorID string, isPinned bool) *data_access.FeedItem {
	now := time.Now()
	return &data_access.FeedItem{
		ContentID:   id,
		AuthorID:    authorID,
		ContentType: 2, // Short
		Status:      2, // Published
...
func TestEnrichWithInteractionStatus_EmptyItems(t *testing.T) {
	...
	result := svc.enrichWithInteractionStatus(context.Background(), "user1", []*data_access.FeedItem{})
	if len(result) != 0 {
...
func TestEnrichWithInteractionStatus_NoUserID(t *testing.T) {
	...
	items := []*data_access.FeedItem{createTestFeedItem("content1", "author1", false)}
