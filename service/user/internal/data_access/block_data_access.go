...
// BlockDataAccess 屏蔽关系数据访问
type BlockDataAccess struct {
	db *sql.DB
}

// NewBlockDataAccess 创建屏蔽数据访问实例
func NewBlockDataAccess(db *sql.DB) *BlockDataAccess {
	return &BlockDataAccess{db: db}
}
...
// Create 创建屏蔽关系
func (r *BlockDataAccess) Create(ctx context.Context, blockerID, blockedID string, blockType BlockType) error {
...
// Delete 删除屏蔽关系
func (r *BlockDataAccess) Delete(ctx context.Context, blockerID, blockedID string) error {
	_, err := r.db.ExecContext(ctx, `
...
// UpdateType 更新屏蔽类型
func (r *BlockDataAccess) UpdateType(ctx context.Context, blockerID, blockedID string, blockType BlockType) error {
	_, err := r.db.ExecContext(ctx, `
...
// Get 获取屏蔽关系
func (r *BlockDataAccess) Get(ctx context.Context, blockerID, blockedID string) (*Block, error) {
	block := &Block{}
...
// Exists 检查屏蔽关系是否存在
func (r *BlockDataAccess) Exists(ctx context.Context, blockerID, blockedID string) (bool, error) {
	var exists bool
	err := r.db.QueryRowContext(ctx, `
...
// GetBlockType 获取屏蔽类型
func (r *BlockDataAccess) GetBlockType(ctx context.Context, blockerID, blockedID string) (BlockType, error) {
...
func (r *BlockDataAccess) IsBlocking(ctx context.Context, blockerID, blockedID string) (bool, BlockType, error) {
	var blockType BlockType
	err := r.db.QueryRowContext(ctx, `
		SELECT block_type FROM blocks WHERE blocker_id = $1 AND blocked_id = $2
	`, blockerID, blockedID).Scan(&blockType)
...
func (r *BlockDataAccess) IsBlockedBy(ctx context.Context, userID, blockerID string) (bool, BlockType, error) {
	return r.IsBlocking(ctx, blockerID, userID)
}

// GetBidirectionalBlockStatus 获取双向屏蔽状态
func (r *BlockDataAccess) GetBidirectionalBlockStatus(ctx context.Context, userID, targetID string) (myBlockType, theirBlockType BlockType, err error) {
...
// GetBlockList 获取屏蔽列表
func (r *BlockDataAccess) GetBlockList(ctx context.Context, userID string, blockType BlockType, limit, offset int) ([]*BlockedUser, int, error) {
	var total int
...
func (r *BlockDataAccess) GetBlockedUserIDs(ctx context.Context, userID string) ([]string, error) {
	rows, err := r.db.QueryContext(ctx, `
...
func (r *BlockDataAccess) GetBlockerUserIDs(ctx context.Context, userID string) ([]string, error) {
	rows, err := r.db.QueryContext(ctx, `
...
func (r *BlockDataAccess) CanViewContent(ctx context.Context, viewerID, targetID string) (bool, error) {
...
func (r *BlockDataAccess) CanSeeContent(ctx context.Context, viewerID, targetID string) (bool, error) {
