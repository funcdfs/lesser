...
func TestFollow_CannotFollowSelf(t *testing.T) {
	// 测试不能关注自己
	userID := "user-123"
	err := repository.ErrCannotFollowSelf
...
func TestFollow_BlockedUser(t *testing.T) {
	// 测试被屏蔽的用户不能关注
	err := repository.ErrFollowBlocked
...
func TestBlock_CannotBlockSelf(t *testing.T) {
	// 测试不能屏蔽自己
	err := repository.ErrCannotBlockSelf
	if !errors.Is(err, repository.ErrCannotBlockSelf) {
...
func TestBlock_InvalidBlockType(t *testing.T) {
	// 测试无效的屏蔽类型
	err := repository.ErrInvalidBlockType
	if !errors.Is(err, repository.ErrInvalidBlockType) {
...
func TestRelationshipStatus_CanViewProfile(t *testing.T) {
	...
	tests := []struct {
		name           string
		theirBlockType repository.BlockType
		expected       bool
	}{
		{"无屏蔽", repository.BlockTypeUnspecified, true},
		{"对方拉黑我", repository.BlockTypeBlock, false},
		{"对方不让我看", repository.BlockTypeHideMe, false},
		{"对方不看我", repository.BlockTypeHidePosts, true},
	}
...
func TestRelationshipStatus_CanBeViewed(t *testing.T) {
	...
	tests := []struct {
		name        string
		myBlockType repository.BlockType
		expected    bool
	}{
		{"无屏蔽", repository.BlockTypeUnspecified, true},
		{"我拉黑对方", repository.BlockTypeBlock, false},
		{"我不让对方看", repository.BlockTypeHideMe, false},
		{"我不看对方", repository.BlockTypeHidePosts, true},
	}
...
func TestDefaultPrivacySettings(t *testing.T) {
	// 测试默认隐私设置
	userID := "user-123"
	settings := repository.DefaultPrivacySettings(userID)
...
func TestDefaultNotificationSettings(t *testing.T) {
	// 测试默认通知设置
	userID := "user-123"
	settings := repository.DefaultNotificationSettings(userID)
...
func TestErrorTypes(t *testing.T) {
	...
	errors := []error{
		...
		repository.ErrUsernameNotFound,
		repository.ErrCannotFollowSelf,
		repository.ErrAlreadyFollowing,
		repository.ErrNotFollowing,
		repository.ErrFollowRequestExists,
		repository.ErrFollowBlocked,
		repository.ErrCannotBlockSelf,
		repository.ErrAlreadyBlocked,
		repository.ErrNotBlocked,
		repository.ErrInvalidBlockType,
		repository.ErrSettingsNotFound,
	}
...
func TestBlockTypes(t *testing.T) {
	// 验证屏蔽类型常量
	if repository.BlockTypeUnspecified != 0 {
		t.Error("BlockTypeUnspecified 应该为 0")
	}
	if repository.BlockTypeHidePosts != 1 {
		t.Error("BlockTypeHidePosts 应该为 1")
	}
	if repository.BlockTypeHideMe != 2 {
		t.Error("BlockTypeHideMe 应该为 2")
	}
	if repository.BlockTypeBlock != 3 {
...
func TestFollowRequestStatus(t *testing.T) {
	// 验证关注请求状态常量
	if repository.FollowRequestPending != 0 {
		t.Error("FollowRequestPending 应该为 0")
	}
	if repository.FollowRequestApproved != 1 {
		t.Error("FollowRequestApproved 应该为 1")
	}
	if repository.FollowRequestRejected != 2 {
