...
func (s *ContentService) Create(
	...
	if isDraft {
		...
		status = repository.ContentStatusDraft
	}
...
// Get 获取内容
func (s *ContentService) Get(contentID, viewerID string) (*repository.Content, error) {
...
func (s *ContentService) Update(
	...
	if err := s.validateContent(content.Type, newTitle, newText, content.Status == repository.ContentStatusDraft); err != nil {
		return nil, err
	}
...
// PublishDraft 发布草稿
func (s *ContentService) PublishDraft(contentID, userID string) (*repository.Content, error) {
	content, err := s.contentRepo.GetByID(contentID)
	if err != nil {
		return nil, err
	}
	...
	if content.Status != repository.ContentStatusDraft {
...
func (s *ContentService) List(
	authorID string,
	contentType repository.ContentType,
	status repository.ContentStatus,
	tags []string,
	limit, offset int,
	orderBy string,
	desc bool,
) ([]*repository.Content, int, error) {
	if limit <= 0 {
		limit = 20
	}
	if limit > 100 {
...
func (s *ContentService) GetUserDrafts(userID string, limit, offset int) ([]*repository.Content, int, error) {
	if limit <= 0 {
		limit = 20
	}
...
// GetReplies 获取回复列表
func (s *ContentService) GetReplies(contentID string, limit, offset int) ([]*repository.Content, int, error) {
	if limit <= 0 {
		limit = 20
	}
...
func (s *ContentService) GetUserStories(userID string) ([]*repository.Content, error) {
	return s.contentRepo.ListUserStories(userID)
}

// BatchGet 批量获取
func (s *ContentService) BatchGet(ids []string) ([]*repository.Content, error) {
...
func (s *ContentService) validateContent(contentType repository.ContentType, title, text string, isDraft bool) error {
	textLen := utf8.RuneCountInString(text)

	switch contentType {
	case repository.ContentTypeShort:
		if text == "" && !isDraft {
	...
	case repository.ContentTypeArticle:
		if !isDraft {
			if title == "" {
	...
	case repository.ContentTypeStory:
		if text == "" && !isDraft {
			return ErrTextRequired
		}
...
func (s *ContentService) UpdateCounter(contentID string, counterType repository.CounterType, delta int32) (int32, error) {
