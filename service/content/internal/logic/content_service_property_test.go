...
func TestProperty9_StoryExpiration(t *testing.T) {
	...
	properties.Property("non-Story content does not have automatic expiration", prop.ForAll(
		func(contentTypeInt int) bool {
			...
			contentType := data_access.ContentType(contentTypeInt%2 + 2) // 2=Short, 3=Article
...
func TestProperty9_ArticleDraftAllowed(t *testing.T) {
	...
	properties.Property("Article draft allows empty title and text", prop.ForAll(
		func(hasTitle, hasText bool) bool {
			...
			// 草稿状态应该允许空标题和空正文
			err := svc.validateContent(data_access.ContentTypeArticle, title, text, true)
	...
	properties.Property("Article published requires title and text", prop.ForAll(
		func(hasTitle, hasText bool) bool {
			...
			err := svc.validateContent(data_access.ContentTypeArticle, title, text, false)
	...
	properties.Property("only Article type supports draft", prop.ForAll(
		func(contentTypeInt int) bool {
			// 测试 Story 和 Short 类型
			contentType := data_access.ContentType(contentTypeInt%2 + 1) // 1=Story, 2=Short
			...
			// 模拟 Create 方法中的逻辑
			if isDraft && contentType != data_access.ContentTypeArticle {
...
func TestProperty10_CounterTypeValidity(t *testing.T) {
	...
	properties.Property("valid counter types are accepted", prop.ForAll(
		func(counterTypeInt int) bool {
			// 生成有效的计数器类型 (1-4)
			counterType := data_access.CounterType(counterTypeInt%4 + 1)

			// 验证：计数器类型在有效范围内
			return counterType >= data_access.CounterTypeLike &&
				counterType <= data_access.CounterTypeBookmark
		},
		gen.IntRange(0, 100),
	))
	...
	properties.Property("invalid counter type is rejected", prop.ForAll(
		func(counterTypeInt int) bool {
			// 生成无效的计数器类型 (0 或 > 4)
			counterType := data_access.CounterType(0)
			if counterTypeInt%2 == 0 {
				counterType = data_access.CounterType(counterTypeInt%10 + 5) // 5-14
			}

			// 验证：计数器类型不在有效范围内
			return counterType == data_access.CounterTypeUnspecified ||
				counterType > data_access.CounterTypeBookmark
		},
		gen.IntRange(0, 100),
	))
