// Package data_access 用户管理 PostgreSQL 数据访问实现
package data_access
...
// UserDataAccessImpl PostgreSQL 用户数据访问（用于管理普通用户）
...
// List 获取用户列表
func (r *UserDataAccessImpl) List(ctx context.Context, filter UserFilter) ([]*User, int, error) {
	// 构建查询条件
...
// GetByID 根据 ID 获取用户
func (r *UserDataAccessImpl) GetByID(ctx context.Context, id uuid.UUID) (*User, error) {
	query := `
		SELECT u.id, u.username, u.email, u.display_name, u.bio, u.avatar_url, u.is_active,
		       COALESCE(ub.is_active, false) as is_banned, ub.reason as ban_reason, ub.expires_at as ban_expires_at,
		       u.followers_count, u.following_count, u.posts_count, u.created_at, u.updated_at
...
// Update 更新用户
func (r *UserDataAccessImpl) Update(ctx context.Context, user *User) error {
	query := `
		UPDATE users 
...
// SoftDelete 软删除用户
func (r *UserDataAccessImpl) SoftDelete(ctx context.Context, id uuid.UUID) error {
...
// HardDelete 硬删除用户
func (r *UserDataAccessImpl) HardDelete(ctx context.Context, id uuid.UUID) error {
...
// Ban 封禁用户
func (r *UserDataAccessImpl) Ban(ctx context.Context, id uuid.UUID, reason string, expiresAt *time.Time, operatorID uuid.UUID) error {
	// 先取消之前的封禁记录
...
// Unban 解封用户
func (r *UserDataAccessImpl) Unban(ctx context.Context, id uuid.UUID) error {
