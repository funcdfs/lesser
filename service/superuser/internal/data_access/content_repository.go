// Package data_access 内容管理 PostgreSQL 仓库实现
package data_access
...
// ContentRepositoryImpl PostgreSQL 内容仓库
type ContentRepositoryImpl struct {
	db *sql.DB
}

// NewContentRepository 创建内容仓库
func NewContentRepository(db *sql.DB) *ContentRepositoryImpl {
	return &ContentRepositoryImpl{db: db}
}

// List 获取内容列表
func (r *ContentRepositoryImpl) List(ctx context.Context, filter ContentFilter) ([]*Content, int, error) {
	// 构建查询条件
...
// GetByID 根据 ID 获取内容
func (r *ContentRepositoryImpl) GetByID(ctx context.Context, id uuid.UUID) (*Content, error) {
	query := `
		SELECT c.id, c.author_id, u.username, c.type, c.status, c.title, c.text, 
		       c.media_urls, c.tags, c.like_count, c.comment_count, c.repost_count, c.view_count,
		       c.created_at, c.published_at
...
// SoftDelete 软删除内容
func (r *ContentRepositoryImpl) SoftDelete(ctx context.Context, id uuid.UUID) error {
...
// BatchDelete 批量删除内容
func (r *ContentRepositoryImpl) BatchDelete(ctx context.Context, ids []uuid.UUID, hard bool) (int, []uuid.UUID, error) {
	if len(ids) == 0 {
		return 0, nil, nil
	}
...
// 确保实现接口
var _ ContentRepository = (*ContentRepositoryImpl)(nil)
