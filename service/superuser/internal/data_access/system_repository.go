// Package data_access 系统监控 PostgreSQL 仓库实现
package data_access
...
// SystemRepositoryImpl PostgreSQL 系统仓库
type SystemRepositoryImpl struct {
	db *sql.DB
}

// NewSystemRepository 创建系统仓库
func NewSystemRepository(db *sql.DB) *SystemRepositoryImpl {
...
func (r *SystemRepositoryImpl) GetSystemStats(ctx context.Context) (*SystemStats, error) {
...
func (r *SystemRepositoryImpl) GetDatabaseStatus(ctx context.Context) (*DatabaseStatus, error) {
...
func (r *SystemRepositoryImpl) GetTableSchema(ctx context.Context, tableName string) (*TableSchema, error) {
...
// ListTables 获取所有表列表
func (r *SystemRepositoryImpl) ListTables(ctx context.Context, schemaName string) ([]string, error) {
	if schemaName == "" {
		schemaName = "public"
	}
...
// ExecuteQuery 执行只读查询
func (r *SystemRepositoryImpl) ExecuteQuery(ctx context.Context, query string, limit int) (*QueryResult, error) {
	// 安全检查：只允许 SELECT 语句
