// Package data_access 内容管理 PostgreSQL 数据访问实现
package data_access
...
// ContentDataAccessImpl PostgreSQL 内容数据访问
type ContentDataAccessImpl struct {
	db *sql.DB
}

// NewContentDataAccess 创建内容数据访问
func NewContentDataAccess(db *sql.DB) *ContentDataAccessImpl {
	return &ContentDataAccessImpl{db: db}
}

// List 获取内容列表
func (r *ContentDataAccessImpl) List(ctx context.Context, filter ContentFilter) ([]*Content, int, error) {
	// 构建查询条件
...
// GetByID 根据 ID 获取内容
func (r *ContentDataAccessImpl) GetByID(ctx context.Context, id uuid.UUID) (*Content, error) {
	query := `
		SELECT c.id, c.author_id, u.username, c.type, c.status, c.title, c.text, 
		       c.media_urls, c.tags, c.like_count, c.comment_count, c.repost_count, c.view_count,
		       c.created_at, c.published_at
...
// SoftDelete 软删除内容
func (r *ContentDataAccessImpl) SoftDelete(ctx context.Context, id uuid.UUID) error {
...
// HardDelete 硬删除内容
func (r *ContentDataAccessImpl) HardDelete(ctx context.Context, id uuid.UUID) error {
...
// BatchDelete 批量删除内容
func (r *ContentDataAccessImpl) BatchDelete(ctx context.Context, ids []uuid.UUID, hard bool) (int, []uuid.UUID, error) {
	if len(ids) == 0 {
		return 0, nil, nil
	}
...
// 确保实现接口
var _ ContentDataAccess = (*ContentDataAccessImpl)(nil)
