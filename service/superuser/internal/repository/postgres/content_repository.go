...
// List 获取内容列表
func (r *ContentRepository) List(ctx context.Context, filter repository.ContentFilter) ([]*repository.Content, int, error) {
	// 构建查询条件
...
// GetByID 根据 ID 获取内容
func (r *ContentRepository) GetByID(ctx context.Context, id uuid.UUID) (*repository.Content, error) {
	query := `
		SELECT c.id, c.author_id, u.username, c.type, c.status, c.title, c.text, 
		       c.media_urls, c.tags, c.like_count, c.comment_count, c.repost_count, c.view_count,
		       c.created_at, c.published_at
