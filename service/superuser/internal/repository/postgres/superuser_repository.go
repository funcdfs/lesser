...
// Create 创建超级管理员
func (r *SuperUserRepository) Create(ctx context.Context, su *repository.SuperUser) error {
	query := `
...
func (r *SuperUserRepository) GetByID(ctx context.Context, id uuid.UUID) (*repository.SuperUser, error) {
	query := `
...
func (r *SuperUserRepository) GetByUsername(ctx context.Context, username string) (*repository.SuperUser, error) {
	query := `
		SELECT id, username, email, password, display_name, is_active, 
		       last_login_at, last_login_ip, login_count, created_at, updated_at
...
func (r *SuperUserRepository) GetByEmail(ctx context.Context, email string) (*repository.SuperUser, error) {
	query := `
		SELECT id, username, email, password, display_name, is_active, 
		       last_login_at, last_login_ip, login_count, created_at, updated_at
...
// Update 更新超级管理员
func (r *SuperUserRepository) Update(ctx context.Context, su *repository.SuperUser) error {
	query := `
		UPDATE superusers 
...
func (r *SuperUserRepository) scanSuperUser(row *sql.Row) (*repository.SuperUser, error) {
