...
type SuperUserService interface {
	...
	// 更新用户
	UpdateUser(ctx context.Context, operatorID uuid.UUID, user *repository.User) (*repository.User, error)

	// ========== 内容管理 ==========
	// 获取内容列表
	ListContents(ctx context.Context, filter repository.ContentFilter) ([]*repository.Content, int, error)
	// 删除内容
	...
	// ========== 审计日志 ==========
	// 获取审计日志
	GetAuditLogs(ctx context.Context, filter repository.AuditLogFilter) ([]*repository.AuditLog, int, error)
}

// LoginResult 登录结果
type LoginResult struct {
	SuperUser    *repository.SuperUser
	AccessToken  string
	RefreshToken string
}
