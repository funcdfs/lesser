...
func TestCreate_NotificationTypes(t *testing.T) {
	...
	tests := []struct {
		...
		{"点赞通知", data_access.NotificationTypeLike, 1},
		{"评论通知", data_access.NotificationTypeComment, 2},
		{"回复通知", data_access.NotificationTypeReply, 3},
		{"收藏通知", data_access.NotificationTypeBookmark, 4},
		{"@提及通知", data_access.NotificationTypeMention, 5},
		{"关注通知", data_access.NotificationTypeFollow, 6},
		{"转发通知", data_access.NotificationTypeRepost, 7},
	}
...
func TestCreate_TargetTypes(t *testing.T) {
	...
	tests := []struct {
		...
		{"内容目标", data_access.TargetTypeContent, "content"},
		{"评论目标", data_access.TargetTypeComment, "comment"},
		{"用户目标", data_access.TargetTypeUser, "user"},
	}
...
func TestMarkAsRead_NotFound(t *testing.T) {
	...
	err := data_access.ErrNotificationNotFound
	if err == nil {
...
func TestErrorTypes(t *testing.T) {
	// 验证预定义错误类型存在
	if data_access.ErrNotificationNotFound == nil {
...
func TestNotification_Fields(t *testing.T) {
	...
	notif := &data_access.Notification{
		ID:         "notif-123",
		UserID:     "user-123",
		Type:       data_access.NotificationTypeLike,
		ActorID:    "actor-456",
		TargetType: data_access.TargetTypeContent,
	...
	if notif.Type != data_access.NotificationTypeLike {
		t.Error("Type 字段不正确")
	}
	if notif.ActorID != "actor-456" {
		t.Error("ActorID 字段不正确")
	}
	if notif.TargetType != data_access.TargetTypeContent {
