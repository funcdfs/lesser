...
### 2.1 API 设计原则
- **对外 API**: 使用 gRPC-Web 通过 Gateway 提供统一 API
- **内部服务通信**: 使用 RabbitMQ 消息队列实现异步处理
- **数据访问**: 各 Worker 服务独立访问数据库

### 2.2 服务划分
- **Gateway**: gRPC API 入口，请求分发到 RabbitMQ
- **Worker 集群**: Auth/Post/Feed/User/Notification/Search/Chat Worker
- **Chat Service**: 高并发实时聊天、WebSocket 连接管理
