...
# 服务管理
dev start              # 启动所有服务
dev start service      # 只启动后端服务 (Django + Chat)
dev start infra        # 只启动基础设施 (PostgreSQL + Redis + Traefik)
dev start client       # 只启动前端客户端
dev stop               # 停止所有服务
dev restart django     # 重启指定服务
dev status             # 查看服务状态
dev logs django        # 查看日志
dev logs django -f     # 实时跟踪日志
...
# 构建和更新
dev build              # 构建镜像
dev rebuild django     # 重新构建 (无缓存)
dev proto              # 生成 Proto 代码
dev update             # 更新环境 (proto + rebuild + migrate)

# 开发调试
dev enter django       # 进入容器
dev shell              # Django Python shell
dev bash django        # 进入容器 bash
