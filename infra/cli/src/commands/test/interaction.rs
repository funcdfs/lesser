...
pub async fn run_tests() -> Result<TestStats> {
    ...
    stats.record_with_func(result, "Bob 点赞内容", "Like()", "service/interaction/internal/handler/interaction_handler.go");
    grpc::delay_short().await;

    // 3. 检查点赞状态
    let result = check_liked(&bob, &content_id).await;
    stats.record_with_func(result, "检查点赞状态", "CheckLiked()", "service/interaction/internal/handler/interaction_handler.go");
    grpc::delay_short().await;

    // 4. Bob 收藏 Alice 的内容
    let result = bookmark_content(&bob, &content_id).await;
    stats.record_with_func(result, "Bob 收藏内容", "Bookmark()", "service/interaction/internal/handler/interaction_handler.go");
    grpc::delay_short().await;

    // 5. Bob 转发 Alice 的内容（带引用）
    let result = create_repost(&bob, &content_id, "好内容，分享给大家！").await;
    stats.record_with_func(result, "Bob 转发内容（带引用）", "CreateRepost()", "service/interaction/internal/handler/interaction_handler.go");
    grpc::delay_short().await;

    // 6. 批量获取交互状态
    let result = batch_get_interaction_status(&bob, &[&content_id]).await;
    stats.record_with_func(result, "批量获取交互状态", "BatchGetInteractionStatus()", "service/interaction/internal/handler/interaction_handler.go");
    grpc::delay_short().await;

    // 7. 获取 Bob 的收藏列表
    let result = list_bookmarks(&bob).await;
    stats.record_with_func(result, "获取收藏列表", "ListBookmarks()", "service/interaction/internal/handler/interaction_handler.go");
    grpc::delay_short().await;

    // 8. Bob 取消点赞
    let result = unlike_content(&bob, &content_id).await;
    stats.record_with_func(result, "Bob 取消点赞", "Unlike()", "service/interaction/internal/handler/interaction_handler.go");
    grpc::delay_short().await;

    // 9. Bob 取消收藏
    let result = unbookmark_content(&bob, &content_id).await;
    stats.record_with_func(result, "Bob 取消收藏", "Unbookmark()", "service/interaction/internal/handler/interaction_handler.go");
    grpc::delay_short().await;

    // 10. Bob 删除转发
    let result = delete_repost(&bob, &content_id).await;
    stats.record_with_func(result, "Bob 删除转发", "DeleteRepost()", "service/interaction/internal/handler/interaction_handler.go");
