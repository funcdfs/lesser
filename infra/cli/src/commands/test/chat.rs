...
pub async fn run_tests() -> Result<TestStats> {
    ...
    stats.record(success, "Alice 发送消息");
    grpc::delay_short().await;

    // 3. Bob 获取会话列表
    let result = get_conversations(&bob).await;
    stats.record(result, "Bob 获取会话列表");
    grpc::delay_short().await;

    // 4. Bob 获取消息列表
    if !conv_id.is_empty() {
        let result = get_messages(&bob, &conv_id).await;
        stats.record(result, "Bob 获取消息列表");
        grpc::delay_short().await;
    } else {
    ...
    if !msg_id.is_empty() {
        ...
        stats.record(result, "Bob 标记消息已读");
        grpc::delay_short().await;
    } else {
    ...
    if !conv_id.is_empty() {
        ...
        stats.record(result, "标记会话所有消息已读");
        grpc::delay_short().await;
    } else {
