...
pub async fn run_tests() -> Result<TestStats> {
    ...
    stats.record_with_func(result, "注册新用户", "Register()", "service/auth/internal/handler/auth_handler.go");
    ...
    stats.record_with_func(result, "重复注册被拒绝", "Register()", "service/auth/internal/handler/auth_handler.go");
    grpc::delay_short().await;

    // 3. 用户登出
    let result = logout_user(&user).await;
    stats.record_with_func(result, "用户登出", "Logout()", "service/auth/internal/handler/auth_handler.go");
    grpc::delay_short().await;

    // 4. 用户登录
    let result = login_user(&mut user).await;
    stats.record_with_func(result, "用户登录", "Login()", "service/auth/internal/handler/auth_handler.go");
    grpc::delay_short().await;

    // 5. 错误密码登录（应该失败）
    let result = test_wrong_password_login(&user).await;
    stats.record_with_func(result, "错误密码登录被拒绝", "Login()", "service/auth/internal/handler/auth_handler.go");
    grpc::delay_short().await;

    // 6. 获取公钥
    let result = get_public_key().await;
    stats.record_with_func(result, "获取 JWT 公钥", "GetPublicKey()", "service/auth/internal/handler/auth_handler.go");
    grpc::delay_short().await;

    // 7. 获取用户信息
    let result = get_user_info(&user).await;
    stats.record_with_func(result, "获取用户信息", "GetUser()", "service/auth/internal/handler/auth_handler.go");
    grpc::delay_short().await;

    // 8. 检查封禁状态
    let result = check_banned(&user).await;
    stats.record_with_func(result, "检查封禁状态", "CheckBanned()", "service/auth/internal/handler/auth_handler.go");
    grpc::delay_short().await;

    // 9. Token 刷新
    let result = refresh_token(&mut user).await;
    stats.record_with_func(result, "刷新 Token", "RefreshToken()", "service/auth/internal/handler/auth_handler.go");
    grpc::delay_short().await;

    // 10. 最终登出
    let result = logout_user(&user).await;
    stats.record_with_func(result, "最终登出", "Logout()", "service/auth/internal/handler/auth_handler.go");
