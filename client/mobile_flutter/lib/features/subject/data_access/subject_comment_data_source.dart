...
// =============================================================================
//
// 实现公共评论组件的 [CommentDataSource] 接口，为剧集动态提供评论数据。
//
// ## 设计说明
//
...
//
// Mock 数据使用两个 Map 存储：
// - `mockComments[postId]` - 根评论列表
// - `mockReplies[parentId]` - 子评论列表
//
// ## 使用示例
//
// ```dart
// final dataSource = SubjectCommentDataSource(
//   subjectId: 'subject_1',
//   currentUserId: 'user_1',
// );
//
// final state = await dataSource.loadComments('post_1', 'series_post');
// ```
...
/// 剧集评论数据源
///
/// 实现 [CommentDataSource] 接口，为剧集动态提供评论数据。
/// 当前使用 Mock 数据，生产环境应替换为 gRPC 实现。
class SubjectCommentDataSource implements CommentDataSource {
  SubjectCommentDataSource({
  ...
  Future<CommentListState> loadComments(
    ...
    final rootComments =
        mockComments[targetId] ?? <subject.SubjectCommentModel>[];

    // 收集所有评论（根评论 + 所有子孙评论）- 扁平化展示
    final allComments = <subject.SubjectCommentModel>[];
  ...
  Future<CommentListState> loadThread(CommentModel rootComment) async {
    ...
    // 递归收集所有子孙评论
    final allDescendants = <subject.SubjectCommentModel>[];
  ...
  void _collectDescendants(
    String parentId,
    List<subject.SubjectCommentModel> result, {
    int depth = 0,
    Set<String>? visited,
  }) {
    // 深度限制检查
  ...
  /// 计算所有评论的子孙数量
  ///
  /// 遍历指定动态的所有根评论，计算每个评论的子孙数量并缓存。
  void _calculateDescendantCounts(String postId) {
    _descendantCounts.clear();

    final comments = mockComments[postId] ?? [];
