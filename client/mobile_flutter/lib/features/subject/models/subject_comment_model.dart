...
// ## 设计说明
//
// 1. **与公共评论组件集成**：通过 `SubjectCommentModelExt` 扩展提供转换方法，
//    将剧集评论转换为公共评论组件可用的格式
//
// 2. **数据与状态分离**：`SubjectCommentModel` 存储业务数据，
...
// - `SubjectCommentModel` - 评论核心模型
// - `CommentUIState` - 评论 UI 状态
// - `SubjectMessageContext` - 动态上下文（评论页头部用）
// - `SubjectCommentInputState` - 评论输入状态
...
class CommentAuthor {
  const CommentAuthor({
    ...
    this.isSubjectOwner = false,
    this.isSubjectAdmin = false,
  });
  ...
  /// 是否是剧集所有者
  final bool isSubjectOwner;

  /// 是否是剧集管理员
  final bool isSubjectAdmin;
  ...
  /// 是否有特殊权限（所有者或管理员）
  bool get hasPrivilege => isSubjectOwner || isSubjectAdmin;

  /// 角色标签（用于 UI 显示）
  String? get roleLabel {
    if (isSubjectOwner) return '剧集主';
    if (isSubjectAdmin) return '管理员';
    return null;
  }
...
///
/// 表示剧集动态下的一条评论，支持嵌套回复。
class SubjectCommentModel {
  const SubjectCommentModel({
    required this.id,
  ...
  /// 切换点赞状态（乐观更新）
  SubjectCommentModel withLikeToggled() => copyWith(
    likeCount: isLiked ? likeCount - 1 : likeCount + 1,
    isLiked: !isLiked,
  );

  /// 回复数 +1（乐观更新）
  SubjectCommentModel withReplyAdded() => copyWith(replyCount: replyCount + 1);

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      (other is SubjectCommentModel && id == other.id);
  ...
  String toString() {
    final preview = content.length > 20
        ...
        : content;
    return 'SubjectCommentModel(id: $id, author: ${author.displayName}, content: $preview)';
  }
}
...
///
/// 用于评论页头部展示原始动态的摘要信息。
class SubjectMessageContext {
  const SubjectMessageContext({
    required this.postId,
    required this.subjectId,
    required this.subjectName,
    required this.subjectAvatarUrl,
...
class SubjectCommentInputState {
  const SubjectCommentInputState({
    this.text = '',
  ...
  /// - 传入 `null`：清除该字段
  /// - 传入具体值：更新为新值
  SubjectCommentInputState copyWith({
    String? text,
    ...
    return SubjectCommentInputState(
      text: text ?? this.text,
  ...
  /// 清空所有输入状态
  SubjectCommentInputState clear() => const SubjectCommentInputState();

  /// 设置回复目标
  SubjectCommentInputState withReplyTo(pkg_comment.ReplyTarget? target) =>
...
extension SubjectCommentModelExt on SubjectCommentModel {
  /// 转换为公共评论模型
