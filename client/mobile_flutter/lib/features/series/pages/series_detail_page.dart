...
import '../handler/series_handler.dart';
import '../models/series_models.dart';
import '../widgets/series_constants.dart';
import '../widgets/series_post.dart' show SeriesPostMenuAction;
...
import 'series_comment_page.dart';
...
class _DetailPageState {
  ...
  final List<SeriesPostModel> posts;
...
/// ## 参数说明
/// - [seriesId]: 剧集 ID（必需）
/// - [initialSeries]: 初始剧集数据（可选，用于 Hero 动画过渡）
...
class SeriesDetailPage extends StatefulWidget {
  ...
  final SeriesModel? initialSeries;
...
class _SeriesDetailPageState extends State<SeriesDetailPage> {
  late final SeriesHandler _handler;
  ...
  void initState() {
    super.initState();
    _handler = SeriesHandler(SeriesMockDataSource());
    ...
    _state = _DetailPageState(
      ...
      isLoading: widget.initialSeries == null,
    );
  ...
  Future<void> _loadData() async {
    ...
    try {
      final series =
          _state.series ?? await _handler.getSeriesDetail(widget.seriesId);

      // 异步操作后检查是否已销毁
      if (_isDisposed) return;

      final posts = await _handler.getPosts(widget.seriesId);
      ...
      setState(() {
        _state = _state.copyWith(
          series: series,
          posts: posts,
          isMuted: _handler.getUIState(widget.seriesId)?.isMuted ?? false,
          isLoading: false,
        );
      });
  ...
  void _openCommentPage(SeriesPostModel post) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => SeriesCommentPage(
          postId: post.id,
  ...
  void _handlePostMenuAction(
    SeriesPostMenuAction action,
    SeriesPostModel post,
  ) {
    switch (action) {
      case SeriesPostMenuAction.save:
        _showSnackBar('动态已保存');
        break;
      case SeriesPostMenuAction.forward:
        _showSnackBar('转发功能开发中');
        break;
      case SeriesPostMenuAction.detail:
  ...
  void _scrollToDate(DateTime date) {
    ...
    if (keyContext != null) {
      Scrollable.ensureVisible(
        keyContext,
        alignment: SeriesLayoutConstants.scrollAlignment,
        duration: SeriesLayoutConstants.scrollDuration,
      ...
      final targetOffset =
          targetIndex * SeriesLayoutConstants.estimatedItemHeight;
  ...
  Widget build(BuildContext context) {
    ...
    final hasPinnedPost = _state.series?.pinnedPost != null;
                ...
                if (_state.showPinnedBanner && hasPinnedPost)
                  Positioned(
                    ...
                    child: PinnedPostBanner(
                      content: _state.series?.pinnedPost?.content ?? '',
  ...
  Widget _buildPostList(AppColorScheme colors, bool hasPinnedPost) {
    ...
    final topPadding =
        ...
        (_state.showPinnedBanner && hasPinnedPost
            ? SeriesLayoutConstants.pinnedBannerHeight
            : SeriesLayoutConstants.defaultTopPadding);
