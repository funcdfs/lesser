...
abstract class SeriesDataSource {
  ...
  /// 用于初始化剧集列表的 UI 状态。
  Future<Map<String, SeriesUIState>> getUIStates();

  /// 获取剧集详情
  ///
  /// 根据剧集 ID 获取完整的剧集信息，包括置顶动态等。
  /// 如果剧集不存在，返回 null。
  Future<SeriesModel?> getSeriesDetail(String id);

  /// 获取剧集动态列表
  ///
  /// 返回指定剧集的动态列表，按时间升序排列（最新在底部）。
  Future<List<SeriesPostModel>> getPosts(String seriesId);

  /// 切换静音状态
  ///
  /// 静音后不会收到该剧集的推送通知，但仍会计算未读数。
  Future<void> toggleMute(String seriesId, bool muted);

  /// 切换置顶状态
  ///
  /// 置顶的剧集会在列表顶部显示。
  Future<void> togglePin(String seriesId, bool pinned);
}
