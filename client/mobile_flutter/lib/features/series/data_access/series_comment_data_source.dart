...
//
// ```dart
// final dataSource = SeriesCommentDataSource(
//   seriesId: 'series_1',
...
class SeriesCommentDataSource implements CommentDataSource {
  SeriesCommentDataSource({
  ...
  Future<CommentListState> loadComments(
    ...
    final rootComments =
        mockComments[targetId] ?? <series.SeriesCommentModel>[];

    // 收集所有评论（根评论 + 所有子孙评论）- 扁平化展示
    final allComments = <series.SeriesCommentModel>[];
  ...
  Future<CommentListState> loadThread(CommentModel rootComment) async {
    ...
    // 递归收集所有子孙评论
    final allDescendants = <series.SeriesCommentModel>[];
  ...
  void _collectDescendants(
    String parentId,
    List<series.SeriesCommentModel> result, {
    int depth = 0,
    Set<String>? visited,
  }) {
    // 深度限制检查
