...
// ## 设计原则
//
// 1. **数据与状态分离**：`SeriesModel` 只包含服务端返回的业务数据，
//    `SeriesUIState` 管理客户端特有的 UI 状态（如未读数、静音、置顶）
//
...
class SeriesModel {
  ...
  /// 置顶动态（可选）
  final SeriesPostModel? pinnedPost;
  ...
  SeriesModel copyWith({
    ...
    return SeriesModel(
      ...
      pinnedPost: pinnedPost == sentinel
          ? this.pinnedPost
          : castOrNull<SeriesPostModel>(pinnedPost),
  ...
  bool operator ==(Object other) =>
      identical(this, other) || (other is SeriesModel && id == other.id);
  ...
  @override
  String toString() =>
      'SeriesModel(id: $id, name: $name, title: $title, subscribers: $subscriberCount)';
}
...
/// 剧集 UI 状态
///
/// 管理剧集列表项的客户端 UI 状态，与核心数据 [SeriesModel] 分离。
class SeriesUIState {
  const SeriesUIState({
  ...
  /// 创建空状态（所有值为默认值）
  factory SeriesUIState.empty(String seriesId) =>
      SeriesUIState(seriesId: seriesId);
  ...
  /// 复制并修改指定字段
  SeriesUIState copyWith({int? unreadCount, bool? isMuted, bool? isPinned}) {
  ...
  @override
  int get hashCode => Object.hash(seriesId, unreadCount, isMuted, isPinned);

  @override
  String toString() =>
      'SeriesUIState(id: $seriesId, unread: $unreadCount, muted: $isMuted, pinned: $isPinned)';
}
