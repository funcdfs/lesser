...
- 再次检查 pkg 和 具体服务之间 的实现。pkg 中不应该包含在其他服务中只会用到一次的包。并且如果 pkg 已经实现了一些东西，其他服务 中就应该不从头实现了。前后端同理，多次用到的函数要放在公共区域
- 不必限时，按照最好的做法来就可以，不要考虑时间问题
- 全面代码审查，检查代码逻辑，是否存在逻辑漏洞，检查代码是否存在性能问题，检查代码是否存在冗余代码，检查代码是否存在不合理的架构方式。作为一个高级程序员视角的角度出发来审查代码。
- 安全与内存检查 。审查代码中是否存在内存泄漏、空指针风险、资源未释放等问题，从安全和稳定性角度审查代码。
- 性能瓶颈分析 ，识别代码中的性能瓶颈，包括频繁的耗时操作、重复计算、未优化的循环、缺失的缓存机制等。
- 不必限时，按照最好的做法来就可以，不要考虑时间问题
- 不必限时，按照最好的做法来就可以，不要考虑时间问题
- 不必限时，按照最好的做法来就可以，不要考虑时间问题
